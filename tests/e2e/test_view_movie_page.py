...
def test_get_single_movie(test_app):
    ...
    assert '<h2>The Matrix</h2>' in data
    assert '<h4>The Wachowski Brothers</h4>' in data
    ...
    if movie_repository.get_movie_by_title("The Matrix").rating < 5:
...
def test_get_single_movie_not_found(test_app):
    response = test_app.get("/movies/-1")
    
    assert response.status_code == 404

def test_get_single_movie_buttons(test_app):
    movie_repository.create_movie("The Matrix", "The Wachowski Brothers", 5)
    movie_id = movie_repository.get_movie_by_title("The Matrix").movie_id
    
    response = test_app.get("/movies") # Back Button
    assert response.status_code == 200
    
    response = test_app.get(f"/movies/{movie_id}/edit") # Edit Button
    assert response.status_code == 200